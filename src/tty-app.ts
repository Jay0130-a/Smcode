...
import { RubricJudge, createDefaultRubric } from './judge/index.js'
import {
  TraceRecorder,
  readRecentTraces,
...
type TtyAppArgs = {
  ...
  /** Optional long-term memory instance (RAG). */
...
async function handleInput(
  ...
  // /evaluate: LLM-as-Judge rubric scoring of the latest turn
  if (input === '/evaluate') {
    await runEvaluateCommand(args, state, rerender)
    return false
  }
  ...
  const localCommandResult = await tryHandleLocalCommand(input, {
    ...
    knowledgeBasePath: args.runtime?.memory.knowledgeBase?.path?.trim() || undefined,
  })
  ...
  // Trace: start a recorder for this turn.
  const traceRecorder = new TraceRecorder({
    cwd: args.cwd,
    sessionId: args.sessionId,
    enabled: args.runtime?.trace.enabled === true,
    maxFileEntries: args.runtime?.trace.maxFileEntries,
  })
  traceRecorder.startTurn(input, args.runtime?.model ?? '')

  // Wire memory/embedding/chroma degradations into this turn's trace.
  args.memory?.setErrorReporter(message => {
  ...
  if (memory?.enabled) {
    await injectMemoryForInput(memory, traceRecorder, args, input)
  }
  ...
  const tracedModel = traceRecorder.active
    ? withTraceModel(args.model, traceRecorder)
    : args.model
  ...
  try {
    const nextMessages = await runAgentTurn({
      ...
      onContextStats(stats) {
        ...
        traceRecorder.setTokenStats(stats)
        rerender()
      },
      async onAutoCompact(result) {
        const compactStartedAt = Date.now()
        traceRecorder.markStage('auto_compact', {
          detail: 'auto',
          input: { tokensBefore: result.tokensBefore, tokensAfter: result.tokensAfter, removedCount: result.removedCount },
          output: { summary: typeof result.summary.content === 'string' ? result.summary.content.slice(0, 200) : '' },
      ...
      async onContextCollapse(result) {
        traceRecorder.markStage('context_collapse', {
          detail: 'collapse',
          output: { collapsed: result.collapsed, spanCount: result.spans.length },
        })
        if (result.collapsed) {
          const savedTokens = await persistContextCollapseResult(args, result)
          state.compressionStatus = `collapse saved ~${Math.round(savedTokens)} tokens`
      ...
      async onSnipCompact(result) {
        traceRecorder.markStage('snip_compact', {
          detail: 'snip',
          output: { removedCount: result.removedMessageIds.length, tokensFreed: result.tokensFreed },
        })
      ...
      onToolStart(toolName, toolInput) {
        traceRecorder.beginStage('tool_call', toolName, toolInput)
        setStatus(state, `Running ${toolName}...`)
        state.activeTool = toolName
        let entryId: number
      ...
      onToolResult(toolName, output, isError) {
        traceRecorder.endStage('tool_call', {
          isError,
    ...
    // Long-term memory: extract important facts at the end of the turn.
    if (memory?.enabled) {
      const added = await memory.extractFromTurn({
        messages: args.messages,
        sessionId: args.sessionId,
      })
      if (added.length > 0) {
        traceRecorder.markStage('memory_extract', {
          detail: 'turn-end',
          output: { added: added.map(entry => entry.id) },
        })
      }
    }

    // Trace: commit the full turn record (must come after extract marking).
...
/**
 * Hybrid BM25 + vector retrieval before a user turn; injects recalled
 * memories as a system message (replacing the previous turn's injection
 * so context does not accumulate). Records rag/rerank/inject trace stages.
 */
async function injectMemoryForInput(
  ...
  if (args.messages.some(message => message.role === 'system' && message.content.startsWith(marker))) {
    args.messages = args.messages.filter(message => !(message.role === 'system' && message.content.startsWith(marker)))
  }

  const topK = memory.config.topK ?? 5
  recorder.beginStage('rag_retrieval', undefined, { query: input, topK })
  const { hits, details } = await memory.search(input, { topK, includeDetails: true })
  recorder.endStage('rag_retrieval', {
    hitCount: hits.length,
    hits: hits.map(hit => ({
      id: hit.entry.id,
      scope: hit.entry.scope,
      score: Number(hit.score.toFixed(3)),
      bm25Score: Number(hit.bm25Score.toFixed(3)),
      vectorScore: Number(hit.vectorScore.toFixed(3)),
      rerankScore: hit.rerankScore !== undefined ? Number(hit.rerankScore.toFixed(3)) : undefined,
    })),
    bm25Top: details?.bm25Top,
    vectorTop: details?.vectorTop,
  })

  recorder.beginStage('rerank', undefined, {
    bm25Top: details?.bm25Top,
    vectorTop: details?.vectorTop,
    fusedTop: details?.fusedTop,
  })
  recorder.endStage('rerank', {
    reranker: memory.rerankerName,
    reranked: hits.map(hit => ({
      id: hit.entry.id,
      fusedScore: Number(hit.score.toFixed(3)),
      rerankScore: hit.rerankScore !== undefined ? Number(hit.rerankScore.toFixed(3)) : undefined,
    })),
  })
  ...
  if (hits.length > 0) {
    ...
    recorder.beginStage('memory_inject', undefined, { hitCount: hits.length, maxChars: memory.config.maxInjectChars })
    args.messages.push({ role: 'system', content: text })
    recorder.endStage('memory_inject', { injected: hits.length, chars: text.length })
  }
}

/** Extract a compact tool-activity summary from a trace for judge evidence. */
function summarizeToolActivity(trace: Trace): string {
  const calls = trace.stages.filter(stage => stage.stage === 'tool_call')
  if (calls.length === 0) return ''
  return calls
    .map(call => {
      const input = typeof call.input === 'string' ? call.input : JSON.stringify(call.input)
      return `- ${call.detail ?? 'tool'}: ${String(input ?? '').slice(0, 200)}`
    })
    .join('\n')
}

/**
 * /evaluate: score the latest turn with the judge LLM against a rubric.
 * The result is appended to the trace log and written as a local report JSON
 * under ~/.mini-code/reports/.
 */
async function runEvaluateCommand(
  ...
  if (!runtime?.model) {
    pushTranscriptEntry(state, {
      kind: 'assistant',
      body: 'No model configured. Cannot evaluate.',
    })
    return
  }
  if (runtime.judge.enabled !== true) {
    pushTranscriptEntry(state, {
      kind: 'assistant',
      body: 'Judge is not enabled. Set "judge": { "enabled": true } in ~/.mini-code/settings.json, then run /evaluate again.',
    })
    return
  }

  const traces = await readRecentTraces(args.cwd, {
    sessionId: args.sessionId,
    limit: 1,
  })
  ...
  if (!trace) {
    pushTranscriptEntry(state, {
      kind: 'assistant',
      body: 'No trace found for this session yet. Run a turn first (trace.enabled must be true), then call /evaluate.',
    })
    return
  }

  const prompt = trace.userInput
  const output =
    trace.finalOutput ?? '（该轮无最终文本输出）'
  const reference = runtime.judge.reference ?? ''
  const evidence = summarizeToolActivity(trace)
  const maxScore = runtime.judge.maxScore ?? 5

  state.isBusy = true
  setStatus(state, 'Evaluating with judge LLM...')
  rerender()
  try {
    const judge = new RubricJudge(args.model, runtime.model, maxScore)
    const result = await judge.evaluate({
      prompt,
      output,
      referenceAnswer: reference.trim() ? reference : undefined,
    ...
    const report = {
      ...
      sessionId: args.sessionId,
      cwd: args.cwd,
      evaluatedAt: result.judgedAt,
      judgeModel: result.model,
      prompt,
      referenceAnswer: reference.trim() ? reference : undefined,
      agentOutput: output,
      evidence: evidence || undefined,
      rubric: createDefaultRubric(maxScore),
      result: {
        ...
        weightedPercent: result.weightedPercent,
        summary: result.summary,
        durationMs: result.durationMs,
      },
    }
    const reportDir = path.join(MINI_CODE_DIR, 'reports')
    await mkdir(reportDir, { recursive: true })
    const reportFile = path.join(reportDir, `evaluate-${Date.now()}-${trace.traceId}.json`)
    await writeFile(reportFile, `${JSON.stringify(report, null, 2)}\n`, 'utf8')

    // Append the judge result to the trace log stream.
    if (runtime.trace.enabled === true) {
      await appendJudgeEventToTrace(args.cwd, args.sessionId, {
        traceId: trace.traceId,
        rubricId: result.rubricId,
        totalScore: result.totalScore,
        maxTotal: result.maxTotal,
    ...
    const scoreLines = result.scores
      .map(score => `- ${score.criterionName}: ${score.score}/${score.maxScore}\n  理由: ${score.rationale}`)
      .join('\n')
    pushTranscriptEntry(state, {
      kind: 'assistant',
      body: [
        `Evaluation (${result.rubricName}) — ${result.weightedPercent}%`,        scoreLines,
        `总分: ${result.totalScore}/${result.maxTotal}`,
    ...
    pushTranscriptEntry(state, {
      kind: 'assistant',
      body: `Evaluation failed: ${error instanceof Error ? error.message : String(error)}`,
    })
  } finally {
    state.isBusy = false
    setStatus(state, null)
